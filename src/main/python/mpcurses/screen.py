...
def validate_screen_size(screen, screen_layout):
    """ validate screen layout is valid for screen size
    """
    ...
    if max_y_pos > screen_height:
        raise Exception(f'the screen is not large enough for the configured layout - make the screen taller')

    if max_x_pos > screen_width:
        raise Exception(f'the screen is not large enough for the configured layout - make the screen wider')
